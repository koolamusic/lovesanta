...
export default function PairPreferenceDrawer({ isOpen, onDrawerClose, pref }: any) {
  return (
    <Drawer placement={'bottom'} onClose={onDrawerClose} isOpen={isOpen} size={'xl'}>
      <DrawerOverlay />
      <DrawerContent minH={'60%'} borderTop={useColorModeValue('1px solid #eee', '1px solid #111')} borderTopRadius={'10px'}>
        <DrawerHeader maxW={'4xl'} mx={'auto'} fontFamily={'heading'}>
          {'Gift bag preferences'}
        </DrawerHeader>
        <DrawerBody fontSize={'sm'} maxW={'4xl'} mx={'auto'}>
          We asked you all what you would like to receive for christmas, I know you are not <strong>Father Christmas 😆 </strong>
          but just in case you know the guy, here are a few things your pair would like:
